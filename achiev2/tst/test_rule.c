...
void test_parcours_en_profondeur(struct board * b) {
   ...
   for(int li=0; li< b->taille; li++) {
    ...
    parcours_en_profondeur(b, 0, 1, 0, connexe_used, &z1);
    /*
    ...
    */
    ...
    if (z1.cases_players[0] == 2) {
        printf("    Le joueur 0 a bien 2 tuiles dans la zone violette\n");
    } else {
        printf("    ERROR : Le joueur 0 a %d tuiles au lieu de 2 dans la zone violette\n", z1.cases_players[0]);
    }

    if (z1.cases_players[1] == 1) {
        printf("    Le joueur 1 a bien 1 tuile dans la zone violette\n");
    } else {
        printf("    ERROR : Le joueur 1 a %d tuiles au lieu de 1 dans la zone violette\n", z1.cases_players[1]);
    }
   ...
   for(int li=0; li< b->taille; li++) {
    ...
    parcours_en_profondeur(b, 0, 2, 1, connexe_used, &z2);
    /*
    ...
    */
    ...
    if (z2.cases_players[0] == 1) {
        printf("    Le joueur 0 a bien 1 tuile dans la zone rouge\n");
    } else {
        printf("    ERROR : Le joueur 0 a %d tuiles au lieu de 1 dans la zone rouge\n", z2.cases_players[0]);
    }

    if (z2.cases_players[1] == 0) {
        printf("    Le joueur 1 a bien 0 tuile dans la zone rouge\n");
    } else {
        printf("    ERROR : Le joueur 1 a %d tuiles au lieu de 0 dans la zone rouge\n", z2.cases_players[1]);
    }
    
    printf("  Fin test parcours_en_profondeur\n");
}

void test_connexe(struct board * b) {
...
void test_zonne_connexe()
{
    ...
    place_tile(&b, t1, p1, 0);
    place_tile(&b, t1, p2, 0);
    place_tile(&b, t2, p3, 1);
    place_tile(&b, t3, p4, 1);
