...
int position_valide(struct board *b, const struct tile * t, int ligne, int colonne) {
    ...
    if (exist_neighbour != -1)  return 1; //On place pas une tuile si il y a pas une tuile a côté
    else return 0;
}
...
void place_tile(struct board * b, const struct tile * t, struct position p, int num_joueur, int nb_players) {
  (b->plateau)[p.X][p.Y] = t; //On place la tuile

  //On changes l'influences des tuiles à côté

    if (p.X > 0 && (b->from_player)[p.X - 1][p.Y][0] != -1) { //En haut
      (b->nb_tuiles_placed_after)[p.X - 1][p.Y] ++; 
      (b->from_player)[p.X - 1][p.Y][num_joueur] += 100 - (10 * (b->nb_tuiles_placed_after)[p.X - 1][p.Y]);     
    }

    if (p.X < b->taille - 1 && (b->from_player)[p.X + 1][p.Y][0] != -1) { //En bas
      (b->nb_tuiles_placed_after)[p.X + 1][p.Y] ++;
      (b->from_player)[p.X + 1][p.Y][num_joueur] += 100 - (10 * (b->nb_tuiles_placed_after)[p.X + 1][p.Y]);     
    }

    if (p.Y > 0 && (b->from_player)[p.X][p.Y - 1][0] != -1) { //A gauche
      (b->nb_tuiles_placed_after)[p.X][p.Y - 1] ++;
      (b->from_player)[p.X][p.Y - 1][num_joueur] += 100 - (10 * (b->nb_tuiles_placed_after)[p.X][p.Y - 1]);     
    }

    if (p.Y < b->taille - 1 && (b->from_player)[p.X][p.Y + 1][0] != -1) { //A droite
      (b->nb_tuiles_placed_after)[p.X][p.Y + 1] ++;
      (b->from_player)[p.X][p.Y + 1][num_joueur] += 100 - (10 * (b->nb_tuiles_placed_after)[p.X][p.Y + 1]); 

    }

    if (p.X > 0 && p.Y > 0 && (b->from_player)[p.X - 1][p.Y - 1][0] != -1) { //Haut gauche
      (b->nb_tuiles_placed_after)[p.X - 1][p.Y - 1] ++;
      (b->from_player)[p.X - 1][p.Y - 1][num_joueur] += 100 - (10 * (b->nb_tuiles_placed_after)[p.X - 1][p.Y - 1]);     
    }

    if (p.X < b->taille - 1 && p.Y > 0 && (b->from_player)[p.X + 1][p.Y - 1][0] != -1) { //Bas gauche
      (b->nb_tuiles_placed_after)[p.X + 1][p.Y - 1] ++;
      (b->from_player)[p.X + 1][p.Y - 1][num_joueur] += 100 - (10 * (b->nb_tuiles_placed_after)[p.X + 1][p.Y - 1]);     
    }

    if (p.X > 0 && p.Y < b->taille - 1 && (b->from_player)[p.X - 1][p.Y + 1][0] != -1) { //Haut droite
      (b->nb_tuiles_placed_after)[p.X - 1][p.Y + 1] ++;
      (b->from_player)[p.X - 1][p.Y + 1][num_joueur] += 100 - (10 * (b->nb_tuiles_placed_after)[p.X - 1][p.Y + 1]);     
    }

    if (p.X < b->taille - 1 && p.Y < b->taille - 1 && (b->from_player)[p.X + 1][p.Y + 1][0] != -1) { //Bas droite
      (b->nb_tuiles_placed_after)[p.X + 1][p.Y + 1] ++;
      (b->from_player)[p.X + 1][p.Y + 1][num_joueur] += 100 - (10 * (b->nb_tuiles_placed_after)[p.X + 1][p.Y + 1]);     
    }

    for (int i = 0; i < nb_players; i ++) { //On met l'influence de tous les joueurs a 0
      (b->from_player)[p.X][p.Y][i] = 0;  
    }

    (b->from_player)[p.X][p.Y][num_joueur] = 100; //On met l'influence du propriétaire a 100
	      ...
	      if (num > 0)
		{
		  for (int i = 0; i < num; i ++) 
		      ...
		      for (int m = 0; m < nb_motifs; m ++) 
			{
		          if (motifs[m].actif) 
			    {
			      if ( (*(motifs[m]).test)(b,ligne,colonne)) //Test si la case ligne colonne contient le motif
				{
        ...
        if (connexe_used[l][c][i] == 0 && b->from_player[l][c][0] != -1) {
          ...
          for(int li=0; li< b->taille; li++) {
            for(int ci=0; ci< b->taille; ci++) {
              connexe_used[li][ci][4] = 0; //On remet a 0 pour pouvoir recompter la case pour le prochain parcours
            }
          }
          parcours_en_profondeur(b, l, c, i, connexe_used, &z, nb_player);

          //Recherches des joueurs avec la plus grande influences
          int max = 0;
...
{
  if (connexe_used[l][c][i] == 0) //test si ce côté de la tuile a déjà été visité
    {
      connexe_used[l][c][i] = 1;

      //test si cette tuile a déjà été visité
      if (connexe_used[l][c][4] == 0)
	{
	  connexe_used[l][c][4] = 1;
	  for (int i = 0; i < nb_players; i ++)
	    {
	      z->cases_players[i] += b->from_player[l][c][i]; //Augmentation de l'influences des joueurs dans la zone

	    }
	  z->nb_tuile += 1; //incrémentation du nombre de tuile dans la zone
	}
      
      for(int k=1; k<4; k++) //Appels sur les autres côtés de la tuile si ils sont de la même couleur
	{
      ...
      switch(i) //Propagation en fonction du côté opposé de la direction du morceau de tuile qu'il étudie
	{
	case 0: //Nord de la tuile propagation sud de la tuile du haut
	...
	case 1: //Sud de la tuile propagation nord de la tuile du bas
	...
	case 2: //Est de la tuile propagation ouest de la tuile de droite
	...
	case 3: //Ouest de la tuile propagation est de la tuile de gauche
...
{
  if (b->from_player[chosed_l][chosed_c][0] != -1) //Test si la case touché est vide
    {
      int proprio[MAX_PLAYERS] = {0};
      int nb_proprietaires = proprietaires(b, chosed_l, chosed_c, nb_players, proprio); //On cherches les joueurs avec l'influence max
    ...
    if (b->from_player[chosed_l][chosed_c][proprio[i]] < 50) {
      ...
      while ( (c_left > -1) && (b->from_player[chosed_l][c_left][0] != -1)) //Les cases à gauche
	{
	  for(int i=0; i<nb_proprietaires; i++)
	    {
      ...
      while ( (c_right < b->taille) && (b->from_player[chosed_l][c_right][0] != -1)) //Les cases à droite
	{
	  for(int i=0; i<nb_proprietaires; i++)
	    {
      ...
      while ( (l_up > -1) && (b->from_player[l_up][chosed_c][0] != -1)) //Les cases en haut
	{
	  for(int i=0; i<nb_proprietaires; i++)
	    {
      ...
      while ( (l_down < b->taille) && (b->from_player[l_down][chosed_c][0] != -1)) //Les cases en bas
	{
	  for(int i=0; i<nb_proprietaires; i++)
	    {
...
int proprietaires(struct board * b, int l, int c, int nb_players, int proprios[]) {
  int max = -10; //Valeur minimale d'une case -1
  int nb_num = 0;
  for (int i = 0; i < nb_players; i ++) {
