...
struct board {
    ...
    int from_player[MAX_BOARD_LONG][MAX_BOARD_LONG][MAX_PLAYERS]; //Influence du joueur sur la case from_player[ligne][colonne][num_joueur]
    int nb_tuiles_placed_after[MAX_BOARD_LONG][MAX_BOARD_LONG]; //Nombre de case placée autours de la case en ligne colonne placé après la case en ligne colonne nb_tuiles_placed_after[ligne][colonne]
};
...
struct player {
    struct file cards; //Les tiles du joueur
    int score;
};

struct zone_connexe {
    int nb_tuile;
    int cases_players [MAX_PLAYERS]; //Somme de l'influence des case de la zone pour le joueur
};
